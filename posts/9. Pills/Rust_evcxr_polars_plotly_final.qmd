...
**Note: I've published all Rust codes as print-only in Quarto markdown file, since it's not possible to run them in RStudio IDE (Rust was not supported). So all Rust codes were run on Jupyter Lab, with code outputs being captured as screenshots, which were shown as photos in this post. I'll try to include a link to the full .ipynb file in my GitHub repository for this portfolio blog, in case anyone wanted to see the .ipynb version.**
...
There was a mention of storing series (column) in a vec in Polars' documentation, which I've tried to plot in Plotly.rs, but unfortunately it failed to recognise it. Then I was searching on StackOverflow for similar questions, then I found something related to what I wanted to do from Polars documentations as shown below.
