...
Due to this reason (note: I did not look into any other optimising strategies for this so this was purely based on the original state of Evcxr at the time), I think Evcxr was not ideal for a very large and complex data science project yet (however if its ecosystem kept developing, it might be improved in the future). One thing of note was that when I was combing through issues in Evcxr's GitHub repository, someone mentioned the slow compile time of the Rust compiler, which would have likely caused the snail speed, but knowing that the actual program running speed was blazingly fast, some sacrifice were needed at the beginning. Overall, Rust was really a systems programming language with memory efficiency (with no garbage collector), type safety and concurrency as its notable advantages.

Because of the dependency loading issue in the Jupyter environment, and also knowing there was already a dataframe library built from Rust, I've opted to use Polars-Python again for the data wrangling part of this project, and then I went on to trial using Rust for data visualisation based on a small dataframe by using Plotly.rs. This project would be separated into 3 parts:
...
A relatively simple dataset would be extracted first for these pills data since I was a very new and inexperienced user of Rust. Therefore, I've selected only certain columns for this purpose.
...
for row in soup.find_all("tr"): 
  ...
  col = row.find_all("td") 
    ...
    # add the texts under columns as specified below
    if (col != []): 
    dosage = col[0].text 
    code = col[1].text 
...
# Create a loop to iterate through the html tags from the soup (scraped html content)
