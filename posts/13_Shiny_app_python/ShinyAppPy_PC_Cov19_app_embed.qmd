...
-   "[Quickstart for R users](https://shiny.rstudio.com/py/docs/r-quickstart.html)" - a very useful link for people who are R users or already familiar with Shiny apps in R, and would like to build Shiny apps in Python
...
One of the downsides of the following code that I couldn't quite get rid of yet was the poor file importation style for loading a .csv file locally. I've added code annotations as comments to explain what I've done. The solution to import .csv file into app.py for the embedding version was from [this discussion](https://github.com/quarto-ext/shinylive/issues/7) in the repository (thanks to that particular user who found this hacky way to do it). However, the plus side was that coding for the rest of the app was not bad at all and worked seamlessly. I'll try to follow up later on whether importing .csv file for app embedding could be easier, perhaps this might be something the Quarto team is working right now.
...
Code for this post (.qmd document) is available [here](https://github.com/jhylin/Data_in_life_blog/blob/main/posts/13_Shiny_app_python/ShinyAppPy_PC_Cov19_app_embed.qmd).
