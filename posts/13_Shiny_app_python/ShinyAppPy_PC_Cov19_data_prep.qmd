...
In this post, I'll talk about the data preparation stage for the final dataset used for building a Shinylive app in Python.
...
##### **Definitions of molecular properties in PubChem dataset**

I thought to devote a specific section on some of the molecular properties listed in the dataset, in case there might be people unfamiliar with this but would still like to read about Shiny app in Python, and this would also serve as a reference for people already know them well.

The definitions of some of the column names were shown below, which were mainly derived and adapted from the documentations in PubChem website:

-   Molecular weight - Molecular mass of compounds measured in daltons

-   Topological polar surface area - measured in angstrom squared

-   Complexity - Complexity rating for compound, based on Bertz/Hendrickson/Ihlenfeldt formula as a rough estimation of how complex a compound is structurally

-   Partition coefficients (xlogp) - Predicted octanol-water partition coefficient which is a measure of hydrophilicity or hydrophobicity of a molecule

-   Heavy atom count - Number of heavy atoms e.g. non-H atoms in the compound
...
-   Rotatable bond count - Defined as any single-order non-ring bond, where atoms on either side of the bond are in turn bound to non-terminal heavy atoms (e.g. non-H). Rotation around the bond axis will change overall molecule shape & generates conformers which can be distinguished by standard spectroscopic methods
...
-   Formal charge - The difference between the number of valence electrons of each atom, and the number of electrons the atom is associated with. Assumes any shared electrons are equally shared between the two bonded atoms.
...
-   Isotope atom count - Number of isotopes that are not most abundant for the corresponding chemical elements. Isotopes = variants of a chemical element that differ in neutron number

-   Defined atom stereocenter count - Atom stereocenter = chiral center = an atom that is attached to 4 different types of atoms or groups of atoms in the tetrahedral arrangement. It can either be (R)- or (S)- configurations. Some of the compounds e.g. racemic mixtures, have undefined atom stereocenter, where (R/S)-config is not specifically defined. This column is the number of atom stereocenters whose configurations are specifically defined

-   Undefined atoms stereocenter count - This is the undefined version of the atoms stereocenter count

-   Defined bond stereocenter count - Bond stereocenter = non-rotatable bond around which two atoms can have different arrangement e.g. in cis- & -trans forms of butene around its double bond. Some compounds have an undefined bond stereocenter (stereochemistry not specifically defined). Defined bond stereocenter count is the number of bond stereocenters whose configurations are specifically defined.

-   Undefined bond stereocenter count - This is the undefined version of the bond stereocenter count
...
Options: 
1. Convert Polars df into Pandas df as per Shiny for Python docs to import dataset in app.

2. Save polars dataframe as .csv file, then read this in the app.py script using Pandas df library
