...
Since I've had a lot of fun building a Shiny app in R last time, I've already planned to build another Shiny app again but using Python instead. So here in this post, I'll talk about the data wrangling process to prepare the final dataset needed to build a Shinylive app in Python. The actual Shinylive app deployment and access will be shown in a separate post after this one.

<br>

##### **Source of data**

The dataset used in for building the Shiny app in Python was from PubChem - [link here](https://pubchem.ncbi.nlm.nih.gov/#tab=compound&query=covid-19%20clinicaltrials). There were a total of 631 compounds at the time when I was building the app. The dataset was simply downloaded as .csv file.
...
The idea was really only keeping all the numerical columns for the sake of showing some data visualisations. So I've dropped all the other columns in texts or the string types.
...
##### **Conditional assignments in Polars - chaining when-then-otherwise expressions**

The longer I've used Polars, the more I realised that it had some very useful coding styles where we could chain a string of different code functions to manipulate dataframes. This in large would avoid writing repeated loop functions to achieve the same results. In this scenario, I'd like to show an example of chaining "when-then-otherwise" expression using Polars. 

<br>

###### **Create different groups of partition coefficients**

So I had the idea of separating all data into 3 different ranges of partition coefficients, so that this could be shown visually in plots. One of the possible ways, or really the long way, to do this might be like the code shown below:
...
A shorter and probably more elegant way was to use the "when-then-otherwise" expression in Polars for conditional assignments (the following code snippet was adapted with thanks to the author of Polars, Ritchie Vink and Stack Overflow):
...
df_name.to_csv("df_name.csv", sep = ",")

# --If preferring to use Polars--
# Simply write a Polars dataframe into a .csv file
df.write_csv("csv_file_name.csv", separator = ",")
```

Once this was done, visit this post to see details on coding and deploying for the app.