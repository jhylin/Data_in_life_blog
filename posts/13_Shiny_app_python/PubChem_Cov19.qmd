...
*** This is the data prepared for building a Shiny app in Python ***
*** Work-in-progress ***
...
pc_cov = pc_cov.with_column(
    pl.when((pl.col("Partition_coef") <= -10))
    .then("Smaller than -10")
    .when((pl.col("Partition_coef") >= -11) & (pl.col("Partition_coef") <= 5))
...
```{python}
fig = px.scatter(x = pc_cov["Partition_coef"], 
                 y = pc_cov["Molecular_weight"], 
                 hover_name = pc_cov["Compound_name"],
...
```{python}
fig = px.scatter(x = pc_cov["Molecular_weight"], 
                 y = pc_cov["Complexity"], 
                 hover_name = pc_cov["Compound_name"],
                 #color = pc_cov["Part_coef_group"],
                 width = 800, 
