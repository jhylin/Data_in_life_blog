...
Since *scikit-learn* mainly supports Pandas dataframes for ML, I've opted to use Pandas instead of Polars dataframe library, to avoid the extra step of converting a Polars dataframe into a Pandas one.
...
To get the LR model ready, we would define our X and y variables from the df_ml dataset.
...
In general, model training could take place initially on the training set, with the validation set used for first evaluation, which would be followed by a final evaluation on the testing set if the model testing worked as expected. The "cross" part of the cross-validation was the part that prevented data loss, in that the training set was splitted into many smaller number (*k*) of sets, which was where cross-validation was often also called "k-fold cross validation". With the use of k-fold cross-validation, the training set was essentially equivalent to *k*-1 of the folds of training data. The trained model would then be validated by using the remaining part of the training data, which was almost like being used as a testing data to measure the performance of the trained model.

::: callout-note
In case above explanation was too long and confusing, in short, cross-validation was simply a commonly used out-of-sample evaluation metric with effective use of data, where each observation was used for both training and testing.
:::

<br>

###### **Decoding LogisticRegressionCV classifer**

Since we've used LogisticRegressionCV classifer for the LR models, this meant it would be unnecessary to use GridSearchCV again if we read the definition of the estimatorCV closely.

Quoted from *scikit-learn* on [cross-validation estimator](https://scikit-learn.org/stable/glossary.html#term-cross-validation-estimator):
...
However, to further dissect LogisticRegressionCV classifer, I wanted to find out what was the cross-validation method actually used. ***Explain Stratified K-folds***
