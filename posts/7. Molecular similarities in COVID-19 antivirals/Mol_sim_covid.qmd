...
date: 2022-11-18
categories: [Data analytics projects, Cheminformatics, Python, RDKit]
jupyter: python3
draft: false
---
...
This work was coded entirely in Python3 and saved as .ipynb file initially in Jupyter Notebook. It was later converted into .qmd file via CLI^[Command line interface], which was run without problems in RStudio IDE. One thing to note was that all 2D chemical structures or similarity maps did not physically show up in RStudio IDE after code executions, but after file rendering, all of them appeared accordingly in the published version.
...
A loop was created to iterate through all 5 antivirals to compare their molecular similarities by using Tanimoto coefficient (TC)^[where 1.0 indicates high similarity, and zero indicates otherwise]. Particularly, each molecule was compared to the other 4 molecules, with results printed as shown below.
...
A side-by-side bar graph showing two different molecular similarity parameters - maximum weights from similarity map and Tanimoto coefficients calculated from Morgan fingerprints - was plotted based on the dataframe created above. It showed similar trend between these two molecular similarity tests for these known COVID-19 antivirals. In that, nirmatrelvir versus ritonavir showed the largest molecular difference out of all 5 compounds with the highest maximum weight. This was reflected in the lowest Tanimoto coefficient as the shortest orange bar, which implied a low similarity between the two molecules. Interestingly, between nirmatrelvir and remdesivir, it appeared the maximum weight was lowest of all 5 molecules, but the Tanimoto coefficient did not quite reflect that (being lower than that for nirmatrelvir versus molnupiravir and baricitinib).
...
Equation:
$$
\text{Tanimoto coefficiant} = \frac{(a \cap b)}{(a + b - (a \cap b))}
$$
TC was summarised as the ratio of the number of chemical features common to two molecules (e.g. molecule *a* and molecule *b*) to the total number of chemical features in the two molecules. 

###### ***Dice similarity/coefficient***

On the other hand, Dice coefficient described the number of features in common for molecules *a* and *b*, relative to the average size of the total number of chemical features present in the two molecules. The weighting factor of 0.5 was shown in the denominator (or can be 2 in the numerator). The coefficient also ranges from zero to 1.
...
As a little prelude to what other work I'm planning to do, I've managed to start learning Rust as well. There is also a back story about why I've started learning Rust, which I'll leave as a probable new post in the future if I feel it fits the context of the post. I think from what I've tried so far, only at seedling stage, it's going to be an even steeper learning curve than Python and R, but I feel it's going to benefit whatever I'm planning to do consciously or unconsciously in the future.