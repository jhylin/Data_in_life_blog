...
##### **Why this post?**
Well, I've always had a thought sitting at the back of my mind about working on a chemistry-related data project since I came back home after my time in Australia. It was also partly due to the research saga mentioned in another [blog post](https://jhylin.github.io/Data_in_life_blog/posts/PhD%20projects/PhD_projects.html). About two months ago, I thought I will never be able to get anything decent (e.g. first-author paper) published out of my PhD, so I was thinking of going down the route of working on health data science, which was where I could tie in my pharmacist background. Miraculously, the paper actually got published last month, which meant I might have more career options now (thank goodness...). 
...
This work was coded entirely in Python3 and saved as .ipynb file initially in Jupyter Notebook. It was later converted into .qmd file via CLI^[Command line interface], which was then run without problems in RStudio IDE. One thing to note was that all 2D chemical structures or simialrity maps did not physically show up in RStudio IDE after code executions, but after file rendering, all of them appeared accordingly in the published version.
...
###### ***molnupiravir***
This was the other oral COVID-19 antiviral, also indicated for mild to moderate symptoms or at risk of severe disease, and also if nirmatrelvir with ritonavir was unsuitable. It was often selected as an oral alternative to nirmatrelvir with ritonavir to avoid drug interactions. It also needed to be taken within 5 days of symptom onset to reach optimal effect.
...
##### **Source of dataset**
The URLs to obtain canonical simplified molecular input line entry systems (SMILES) of all 5 molecules are listed below (there are several different options to obtain SMILES for molecules, I've decided to use PubChem in this case):
...
A loop was created to iterate through all 5 antivirals to compare their molecular similarities by using Tanimoto coefficient (TC)^[where 1.0 indicates high similarity, and zero indicates otherwise]. Particularly, each molecule was compared to the other four molecules, with results printed as shown below.
...
A side-by-side bar graph showing two different molecular similarity parameters - maximum weights from similarity map and Tanimoto coefficients calculated from Morgan fingerprints - was plotted based on the dataframe created above. It showed similar trend between these two molecular similarity tests for these known COVID-19 antivirals. In that, nirmatrelvir versus ritonavir showed the largest molecular difference out of all 5 compounds with the highest of maximum weight, which was reflected in the seemingly lowest Tanimoto coefficient (shortest orange bar) shown. Interestingly, between nirmatrelvir and remdesivir, it appeared its maximum weight was lowest of all 5 molecules, but the Tanimoto coefficient did not quite reflect that (being lower than that for nirmatrelvir versus molnupiravir and baricitinib), showing that there could be some variances in the similarity parameters. 
...
However. if we bring back the dataframe itself and have a look again:

```{python}
df_ms_new
```
All 3 molecular comparisons' T_coeff from rows 1 to 3 showed very similar results for T_coeff if we rounded the results to two decimal places. Also, this was probably not a very good example of using these two RDKit molecular similarity parameters  due to the very small sample size.

<br>

##### **Acknowledgement**

The codes used in this post were heavily inspired by and adapted from the following blogs and website shown below. I've learnt a lot from them, and would like to thank their existence to help readers like me to learn in an open-source manner. I particularly like the clear and concise writing style from P. Walter's Practical Cheminformatics blog which is easy to follow, whereas Iwatobipen's is life worth it? blog is full of surprises to show a wide diverse range of cheminformatics tools available to use.
...
I have read quite a few blog posts from P. Walter and Iwatobipen, and have enjoyed them but never quite got around to write one myself, so finally I did it... although this post itself was not written in a grand scale, I hope to project what I did here in the future, e.g. to apply them to a larger set of compounds. My very draft thought now is to perhaps trial using ChEMBL library, which from what I've heard, is a useful open-source cheminformatics compounds library. 

As a little prelude to what other work I'm also planning to do, um... I've managed to start learning Rust as well (there is also a story about why I started learning Rust, which I'll leave for another future post to explain if I feel it fits the context of the post. I think from what I tried so far, only at the seedling stage currently, it's going to be an even steeper learning curve than Python and R, but I feel it's going to benefit whatever I'm planning to do consciously/unconsciously in the future).