...
![Where to download small molecules from ChEMBL database](ChEMBL_cpds.jpg)
...
The plan was to use logistic regression method for ML model, so the y variable I was interested in here was whether a small molecule would be approved or not, so it was going to be a binary categorical variable - meaning it needed to be 0 (not approved) or 1 (approved).
To do this, add a new column with a new name of "Max_Phase" and replace "4" as "1" by dividing by 4 to reach this new label.
...
y_mp_prob.describe()
```
###### **Accuracy score for the LR classifier**

One caveat to note was that for imbalanced dataset, accuracy score might not be very informative, consider using other evaluation metrics. Here, we had an accuracy score of 0.7, which was not too low but also not extremely high either.
...
This was something I thought to try when I was reading through *scikit-learn* documentation.
