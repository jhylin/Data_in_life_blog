...
```{python}
#| code-fold: true
# see comment below re. warning message for OpenMP runtimes
import warnings
...
# I've decided to ignore this warning in the meantime as this is a blog post as demonstration only,
...
# 24/6/25: solved the mystery regarding ChEMBL version 35 not loading properly earlier this year, 
# it has been continuously showing an error message such as 
# "Database disk image is malformed" every time trying to use v35
# it was not due to ipython history issue 
# (mistakenly written down as sqlite history previously which was not right)
# turned out that the old copy of downloaded ChEMBL v35 might be a bit corrupted 
# so it was removed with a new one re-downloaded then it worked!
```
