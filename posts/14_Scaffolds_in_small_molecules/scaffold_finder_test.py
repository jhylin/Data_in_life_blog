...
# Tweaked version 1
# Changed activity_df to cpd_df as there were no detailed compound activity data involved yet
# **Only wanted to make a general comparison of a potential hit to current Rx anti-bacterials
# with similar pathogenic targets (Gram +ve)**

def get_molecules_with_scaffold(scaffold, mol_df, cpd_df):
    """
    ...
    """
    match_df = mol_df.query("Scaffold in @scaffold")
    merge_df = match_df.merge(cpd_df, on=["SMILES", "Name"])
    ...
    if len(rgroup_match):
        ...
        return rgroup_df.Core.unique(), merge_df[["SMILES", "Name"]]
    else:
        return [], merge_df[["SMILES", "Name"]]
