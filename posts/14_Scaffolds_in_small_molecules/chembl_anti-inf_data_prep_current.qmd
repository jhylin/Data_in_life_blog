...
Quick overview on all column names, along with their data and data types.

```{python}
print(df_ai.glimpse())
```

Under the "Availability Type" column, there were a few different availability for each anti-bacterial such as, "Discontinued", "Withdrawn", "Unknown" and "Prescription Only".

```{python}
df_ai.groupby("Availability Type").count()
```

Because I'd like to choose only the "Prescription Only" ones, I then added the following filter condition.
...
In preparation of possible future work on building machine learning models along this line of work, I looked into datamol's pre-processing molecule function (shown in next section), as it involved converting SMILES strings into SELFIES, which were considered more robust than SMILES. However, I kept running into a code error, where the error message showed the SMILES column was empty. This took me a few tries to realise that I've actually forgotten to check whether there were any missing SMILES in the same column. So below I've filtered the SMILES column to look for any missing cells.

```{python}
df_ai_rx.filter(pl.col("Smiles") == "")
```

Neomycin was found to have no SMILES recorded. To fix this error, I found another useful way to use the previously-mentioned "when-then-otherwise" expression in Polars again to replace empty strings in dataframes. The code example below was kindly adapted from StackOverflow with [link here](https://stackoverflow.com/questions/72292048/idiomatic-replacement-of-empty-string-with-pl-null-null-in-polars).
...
The "Smiles" column name was changed to ensure _preprocess function would work since the parameter smiles_column was "smiles" with lowercase "s". The "Name" column was changed accordingly for similar reason.
...
I also thought to change the capitalised compound names into lowercases, since I'd prefer to read these compounds names in a more friendly way (rather looking like they were shouting at me being all capitalised letters).
...
As usual, since Datamol was built as a thin layer library on top of RDKit, which was only really compatible with Pandas, I've added this step to convert the dataframe into a Pandas one.
...
I have basically borrowed and adapted the _preprocess function from Datamol ([link here](https://docs.datamol.io/stable/tutorials/Preprocessing.html)), as shown below - to show that it also included a conversion from "mol" (RDKit molecule) to SELFIES (self-referencing embedded strings) as well.
...
##### **Converting SMILES into SELFIES**

For people who might be interested or new to this type of work, there were two ways to convert multiple SMILES into multiple SELFIES (there might be more, but I've found these two for now):
