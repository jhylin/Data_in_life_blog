...
::: callout-note
In short, cross-validation was commonly used as an out-of-sample evaluation metric, where each observation was used for both training and testing, leading to effective use of data.
:::
...
# Set up grid search
# Specify grid for parameters to test
...
grid_search = GridSearchCV(
  ...
  param_grid = grid,
  # Number of jobs to run in parallel (1 = no parallel jobs; None = unset, but could be interpreted as "1" unless otherwise specified; -1 = all processors used)
  n_jobs = -1,
  # Type of cross-validation method to be used (if none set, default 5-fold cv will be used)
  cv = CV, 
  # Type of scoring to be used to evaluate the model
  scoring = "accuracy",
  # Value to assign to the "scoring" of the model if an error occurs during model fitting
  error_score = 0
  )
...
# Results with means & standard deviations of accuracy scores with parameters used
...
From the grid search, the values of Cs and cv parameters that generated the best mean accuracy score were 50 and 5 respectively. The best mean accuracy score produced was 0.699263 with a standard deviation of 0.043226. Solver-wise, there were actually 3 other solvers, liblinear, newton-cg and newton-cholesky, in addition to the default one (lbfgs), that also generated the same mean accuracy score with the same standard deviations, while using Cs = 50 and cv = 5. In my initial LogisticRegressionCV model, I have used a different value of Cs parameter (at 10), but with the same cv and penalty parameters. For the next ML series 3, I was planning to re-train the LogisticRegressionCV model with the found best parameters and do a final round of evaluation, and see if there would be any particular differences, although I suspected the difference might be small, as shown from the current mean accuracy score found from this grid search. However, the goal of this post was to understand how cross-valiadation could be used for hyper-parameter tuning to find the optimal parameters to avoid overfitting a ML model.
...
::: callout-note
Feel free to skip this final part as this was really me speaking my thoughts out loud about my situation lately.
:::
