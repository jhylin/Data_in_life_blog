...
This post was really just an addition towards the last random forest (RF) post. It was mainly inspired by this paper [@esposito2021] from [rinikerlab](https://github.com/rinikerlab)[^1]. It was nice to complete the RF series by adding a RF classifier model since last post was only on a regressor. Another thing was that imbalanced datasets were common in drug discovery projects, learning different strategies to deal with them was also very useful. While working on this post, I also came across a few other packages that I haven't used before so I've included them all down below.
...
-   Minor data preprocessing using own little script of mol_prep.py and also a SMILES checker from *scikit_mol*
...
Minor cleaning and preprocessing were done for this post only, as the focus was more on dealing with imbalanced datasets in RF classifier. Since I used a different way to retrieve ChEMBL data this time, the datasets used here might be slightly different from the one used in previous post.

<br>

###### **mol_prep.py**

I've more or less accumulated small pieces of code over time, and I've decided to compile them into a Python script. The idea was to remove most function code in the post to avoid repeating them all the time, as they've been used frequently in the last few posts. The script would be saved into the [RF repository](https://github.com/jhylin/ML2-2_random_forest). It would still be a "work-in-progress" script (needs more work in the future).
...
Some reference links and explanations for area under the receiver operating characteristic (ROC) curve and Cohen's Kappa.
...
Note: roc_auc_score measures true positive and false positive rates, so it requires binary labels (e.g. 0s and 1s) in the data
...
It was very obvious that not all compounds were classified in the testing set. There were only 1052 compounds classified as true negative, and none in the testing set were labelled as true positive. The likely reason was due to the very imbalanced ratio of actives (only 10 max phase 4 which were labelled as "1" compounds) and inactives (5256 max phase 0 compounds). Besides the imbalanced dataset, the decision threshold was also usually set at 0.5, this meant the classifier would lose the chance to classify the true positive compounds due to the very small proportion of the active molecules. 
...
Here, after shifting the decision threshold with the most optimal Cohen's Kappa score, we could see an improved number of compounds labelled within the true negative class (increasing from 1052 to 4204). The true positive class had also improved from 0 to 7.
...
I'd like to thank Riniker lab again for the GHOST paper, and also all the authors, contributors or developers of all the software packages mentioned in this post. Last but not the least, huge thanks should also go to the authors of all the reference notebooks mentioned in this post as well.