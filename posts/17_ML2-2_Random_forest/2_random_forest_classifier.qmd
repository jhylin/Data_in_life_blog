...
The aim of this post was to model and classify the max phases of ChEMBL small molecules, i.e. whether the compounds in the testing set (consisted of max phase 0 or null compounds) might be eventually classified as max phase 4 or not. The target was "max_phase" and features to be used were the various RDKit 2D (RDKit2D) descriptors.
...
# I decided to show it explicitly in the code 
...
Some references links for area under the receiver operating characteristic (ROC) curve and Cohen's Kappa.

Area under the ROC curve: [reference link](https://developers.google.com/machine-learning/crash-course/classification/roc-and-auc)

Cohen's Kappa score: [reference link](https://scikit-learn.org/stable/modules/generated/sklearn.metrics.cohen_kappa_score.html#r219a3b9132e1-1)

```{python}
# Rather than re-inventing the wheel, 
# the following function code for calculating metrics of the RF model 
# was adapted from Landrum et al., from this notebook: 
# https://github.com/rinikerlab/GHOST/blob/main/notebooks/example_GHOST.ipynb
# Some comments were added for clarities
...
**TODO**
Mention confusion matrix results - TP, FP etc.
...
5.  two approaches from GHOST (generalized threshold shifting) paper (main idea was optimising and shifting decision threshold):
    ...
    -   Approach 2 led to generalised threshold shifting (GHOST) procedure (for any classification methods) - GitHub repository: [https://github.com/rinikerlab/GHOST](https://github.com/rinikerlab/GHOST)

I only used approach 2 here as both approaches were shown to be performing similarly in the paper, and the RDKit blog post mentioned above already had a detailed explanation for approach 1.
...
6.  Shifting decision threshold by using ghostml in a postprocessing way (note: last post used re-sampling method in a preprocessing way)

The decision threshold were optimised by using ghostml via testing various different thresholds, e.g. in spaces of 0.05 that ranged from 0.05 to 0.5. The most optimal threshold would have the maximised Cohen's kappa.
...
I'd like to thank [rinikerlab](https://github.com/rinikerlab) again, and also all the authors, contributors or developers of all the software packages mentioned in this post, and all the authors of the reference notebooks I've referred to in this post too.