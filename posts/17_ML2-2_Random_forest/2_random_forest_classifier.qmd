...
The two dataframes should share the same column names to be combined together.
...
A useful web link on area under the receiver operating characteristic (ROC) curve - https://developers.google.com/machine-learning/crash-course/classification/roc-and-auc

```{python}
# Rather than re-inventing the wheel, 
# the following function code for calculating metrics of the RF model 
# was adapted from Landrum et al., 
# from this notebook: https://github.com/rinikerlab/GHOST/blob/main/notebooks/example_GHOST.ipynb
...
Note: roc_auc_score measures true positive and false positive rates, therefore needs to have binary labels (e.g. 0s and 1s) in the data
...
5.  two approaches from GHOST (generalized threshold shifting) paper (main idea was optimising and shifting decision threshold):
    ...
    -   approach 1 (out-of-bag method, more computer efficient, aimed for RF classifiers) based on RDKit blog post (ref. 41) - http://rdkit.blogspot.com/2018/11/working-with-unbalanced-data-part-i.html ([viewable notebook version](https://nbviewer.org/github/greglandrum/rdkit_blog/blob/master/notebooks/Working%20with%20unbalanced%20data%20part%201.ipynb) via nbviewer)
...
I only used approach 2 here as both approaches were shown to be performing similarly in the paper, and also approach 1 was already described in the RDKit blog post as mentioned above.
