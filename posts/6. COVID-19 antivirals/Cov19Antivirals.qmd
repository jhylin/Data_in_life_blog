...
library(knitr)
```
Read the .csv file downloaded from ClinicalTrials.gov
```{r}
df <- read_csv("Cov19_studies.csv")
df
```
...
spec(df)
```
Check how many studies are still active and if results are available.
```{r}
df_status <- df %>% 
  group_by(Status, `Study Results`) %>% 
  summarise(number = n())
df_status
```
Filter out terminated studies and show their status and intervention details.
```{r}
df_status_studies <- df %>% 
  filter(Status == "Terminated") %>% 
  select(Status, Interventions)
kable(df_status_studies)
```
Interestingly, three out of 15 terminated studies involved hydroxychloroquine and two out of the terminated studies involved testing convalscent plasma.
